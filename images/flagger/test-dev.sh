#!/bin/bash
# Smoke test for minimal-flagger-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing flagger binaries present (parity with prod)..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/flagger"
...
echo "✓ All flagger-dev smoke tests passed"
