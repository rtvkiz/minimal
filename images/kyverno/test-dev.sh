#!/bin/bash
# Smoke test for minimal-kyverno-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing kyverno binaries present (parity with prod)..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/kyverno"
...
echo "✓ All kyverno-dev smoke tests passed"
