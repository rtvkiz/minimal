#!/bin/bash
# Smoke test for minimal-victoria-logs-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing victoria-logs binaries present (parity with prod)..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/victoria-logs"
...
echo "✓ All victoria-logs-dev smoke tests passed"
