#!/bin/bash
# Smoke test for minimal-reloader-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing reloader binaries present (parity with prod)..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/reloader"
...
echo "✓ All reloader-dev smoke tests passed"
