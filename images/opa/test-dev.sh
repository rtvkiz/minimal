#!/bin/bash
# Smoke test for minimal-opa-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing opa version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/opa "$IMAGE" version 2>&1 | grep -qiE 'Version:[[:space:]]*v?1\.[0-9]'
...
echo "✓ All opa-dev smoke tests passed"
