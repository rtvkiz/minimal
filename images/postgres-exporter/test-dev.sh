#!/bin/bash
# Smoke test for minimal-postgres-exporter-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing postgres-exporter binaries present (parity with prod)..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/postgres_exporter"
...
echo "✓ All postgres-exporter-dev smoke tests passed"
