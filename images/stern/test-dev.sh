#!/bin/bash
# Smoke test for minimal-stern-dev.
set -eu

: "${IMAGE:?IMAGE env var required}"

echo "Testing stern version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/stern "$IMAGE" --version 2>&1 | grep -qiE '1\.[0-9]+'
...
echo "✓ All stern-dev smoke tests passed"
