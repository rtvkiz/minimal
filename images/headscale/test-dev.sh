#!/bin/bash
# Smoke test for minimal-headscale-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing headscale binaries present (parity with prod)..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/headscale"
...
echo "✓ All headscale-dev smoke tests passed"
