#!/bin/bash
# Smoke test for minimal-flux-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing flux version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/flux "$IMAGE" --version 2>&1 | grep -qiE '2\.[0-9]+'
...
echo "✓ All flux-dev smoke tests passed"
