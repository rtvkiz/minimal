#!/bin/bash
# Smoke test for minimal-external-secrets-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing external-secrets binaries present (parity with prod)..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/external-secrets"
...
echo "✓ All external-secrets-dev smoke tests passed"
