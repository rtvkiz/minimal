#!/bin/bash
# Smoke test for minimal-pulsar-dev.
...
echo "Testing pulsar entrypoint exists..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/pulsar-entrypoint.sh"

echo "Testing pulsar launcher shipped..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /opt/pulsar/bin/pulsar"
...
echo "✓ All pulsar-dev smoke tests passed"
