#!/bin/bash
# Smoke test for minimal-solr-dev.
...
echo "Testing solr entrypoint exists..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /usr/bin/solr-entrypoint.sh"

echo "Testing solr launcher shipped..."
docker run --rm --entrypoint /bin/sh "$IMAGE" -c "test -x /opt/solr/bin/solr"
...
echo "✓ All solr-dev smoke tests passed"
