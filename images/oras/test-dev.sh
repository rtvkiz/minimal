#!/bin/bash
# Smoke test for minimal-oras-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing oras version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/oras "$IMAGE" version 2>&1 | grep -qiE 'Version:[[:space:]]*v?1\.[0-9]'
...
echo "✓ All oras-dev smoke tests passed"
