#!/bin/bash
# Smoke test for minimal-nats-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing nats-server version (parity with prod)..."
docker run --rm "$IMAGE" --version | grep -qE "nats-server"
...
echo "✓ All nats-dev smoke tests passed"
