#!/bin/bash
# Smoke test for minimal-kubeseal-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing kubeseal version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/kubeseal "$IMAGE" --version 2>&1 | grep -qiE '0\.[0-9]+'
...
echo "✓ All kubeseal-dev smoke tests passed"
