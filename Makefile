...
.PHONY: etcd etcd-melange victoria-metrics victoria-metrics-melange jaeger jaeger-melange otelcol otelcol-melange qdrant qdrant-melange meilisearch meilisearch-melange meilisearch-dev deno deno-melange
...
#------------------------------------------------------------------------------
# MEILISEARCH IMAGE (Rust source build + apko)
#------------------------------------------------------------------------------
meilisearch-melange: keygen
	@echo "Building Meilisearch $(MEILISEARCH_VERSION) from source via melange (Rust)..."
	melange build images/meilisearch/melange.yaml \
		--arch x86_64 \
		--signing-key melange.rsa
	@echo "✓ Meilisearch package built from source"

meilisearch: meilisearch-melange
	@echo "Assembling minimal-meilisearch image with apko..."
	apko build images/meilisearch/apko/meilisearch.yaml \
		$(REGISTRY)/$(OWNER)/minimal-meilisearch:$(VERSION) \
		meilisearch.tar \
		--arch x86_64 \
		--repository-append ./packages \
		--keyring-append melange.rsa.pub
	docker load < meilisearch.tar
	docker tag $(REGISTRY)/$(OWNER)/minimal-meilisearch:$(VERSION)-amd64 \
		$(REGISTRY)/$(OWNER)/minimal-meilisearch:$(VERSION)
	docker tag $(REGISTRY)/$(OWNER)/minimal-meilisearch:$(VERSION)-amd64 \
		$(REGISTRY)/$(OWNER)/minimal-meilisearch:latest
	@rm -f meilisearch.tar sbom-*.spdx.json
	@echo "✓ minimal-meilisearch built (Rust source build)"
...
test-meilisearch:
	@echo "Testing Meilisearch image..."
	export IMAGE="$(REGISTRY)/$(OWNER)/minimal-meilisearch:latest" && \
		images/meilisearch/test.sh
	@echo "✓ Meilisearch tests passed"
