#!/bin/bash
# Smoke test for minimal-osv-scanner-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing osv-scanner version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/osv-scanner "$IMAGE" --version 2>&1 | grep -qiE '2\.[0-9]+'
...
echo "✓ All osv-scanner-dev smoke tests passed"
