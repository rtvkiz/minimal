#!/bin/bash
# Smoke test for minimal-traefik-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing traefik version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/traefik "$IMAGE" version 2>&1 | grep -qE "Version:"
...
echo "✓ All traefik-dev smoke tests passed"
