#!/bin/bash
# Smoke test for minimal-gitleaks-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing gitleaks version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/gitleaks "$IMAGE" version 2>&1 | grep -qiE '(v?)8\.[0-9]'
...
echo "✓ All gitleaks-dev smoke tests passed"
