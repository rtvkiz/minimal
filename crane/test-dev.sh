#!/bin/bash
# Smoke test for minimal-crane-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing crane version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/crane "$IMAGE" version 2>&1 | grep -qiE '0\.[0-9]+'
...
echo "✓ All crane-dev smoke tests passed"
