...
emit 'family_sweep() {'
emit '  _g="$1"; _new="$3"; _p="$4"'
emit '  # Match the family at ANY version, not just the one the scan flagged.'
emit '  # jackson-annotations sat at 2.20 (Jackson versions it major.minor) while'
emit '  # jackson-databind went to 2.21.5, so a sweep keyed on the flagged version'
emit '  # never saw it — and Solr died with NoClassDefFoundError JsonSerializeAs.'
