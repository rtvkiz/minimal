...
emit '    # Per-file copy: a group resolved for one artifact must not leak into'
emit '    # the next iteration.'
emit '    _gg="$_g"'
emit '    _url="https://repo1.maven.org/maven2/$(echo "$_gg" | tr . /)/$_a/$_new/$_a-$_new.jar"'
emit '    if ! curl -sfI --connect-timeout 10 --max-time 30 "$_url" -o /dev/null; then'
emit '      # A prefix match does not guarantee the same group: jackson-databind'
emit '      # is com.fasterxml.jackson.core, but jackson-module-jakarta-xmlbind-'
emit '      # annotations is com.fasterxml.jackson.module. For these split'
emit '      # families the group tracks the artifact id: swap the last group'
emit '      # segment for the artifact second token and try once more.'
emit '      # search.maven.org would answer this directly but is not reliably'
emit '      # reachable from a build sandbox, so it is not a dependency here.'
emit '      _g2="${_gg%.*}.$(echo "$_a" | cut -d- -f2)"'
emit '      _u2="https://repo1.maven.org/maven2/$(echo "$_g2" | tr . /)/$_a/$_new/$_a-$_new.jar"'
emit '      if curl -sfI --connect-timeout 10 --max-time 30 "$_u2" -o /dev/null; then'
emit '        _gg="$_g2"; _url="$_u2"'
emit '      else'
emit '        # Genuinely unrelated — calcite-core and opentelemetry-semconv both'
emit '        # sat at 1.37.0. Refuse rather than invent coordinates.'
emit '        echo "patch-java-deps: $_b left at $_old, not under $_gg or $_g2" >&2'
emit '        exit 1'
emit '      fi'
emit '    fi'
emit '    curl -sSfL --connect-timeout 10 --max-time 120 --retry 3 --retry-delay 5 \'
