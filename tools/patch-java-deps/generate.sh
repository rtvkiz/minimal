...
emit '      # Split families put the artifact second token in the group, but not'
emit '      # consistently: jackson REPLACES the last segment'
emit '      #   (com.fasterxml.jackson.core -> com.fasterxml.jackson.module),'
emit '      # while jetty APPENDS it'
emit '      #   (org.eclipse.jetty -> org.eclipse.jetty.http2).'
emit '      # Try both shapes. search.maven.org would answer directly but times'
emit '      # out from a build sandbox, so it is not a dependency here.'
emit '      _tok=$(echo "$_a" | cut -d- -f2)'
emit '      _ok=0'
emit '      for _cand in "${_gg%.*}.$_tok" "$_gg.$_tok"; do'
emit '        _u2="https://repo1.maven.org/maven2/$(echo "$_cand" | tr . /)/$_a/$_new/$_a-$_new.jar"'
emit '        if curl -sfI --connect-timeout 10 --max-time 30 "$_u2" -o /dev/null; then'
emit '          _gg="$_cand"; _url="$_u2"; _ok=1; break'
emit '        fi'
emit '      done'
emit '      if [ "$_ok" -eq 0 ]; then'
...
emit '        echo "patch-java-deps: $_b left at $_old, no group found for $_a-$_new" >&2'
emit '        exit 1'
emit '      fi'
emit '    fi'
emit '    curl -sSfL --connect-timeout 10 --max-time 120 --retry 3 --retry-delay 5 \'
