#!/bin/bash
# Smoke test for minimal-sops-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing sops version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/sops "$IMAGE" --version 2>&1 | grep -qiE '3\.[0-9]+'
...
echo "✓ All sops-dev smoke tests passed"
