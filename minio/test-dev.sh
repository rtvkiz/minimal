#!/bin/bash
# Smoke test for minimal-minio-dev.
set -eu  # NB: no pipefail — `docker run | grep -q` is SIGPIPE-prone in CI

: "${IMAGE:?IMAGE env var required}"

echo "Testing minio version (parity with prod)..."
docker run --rm --entrypoint /usr/bin/minio "$IMAGE" --version 2>&1 | grep -qE "minio version"
...
echo "✓ All minio-dev smoke tests passed"
